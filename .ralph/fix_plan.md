...
- [ ] Build result page (`/result/[id]`) — verdict display (SELL/NOT_SELL), confidence indicator, financial breakdown table, AI reasoning bullets, CTAs

## Medium Priority
- [ ] Add Canadian-specific validation (phone +1, postal code format, province auto-fill from address)
- [ ] Implement capital gains tax logic with principal residence exemption (full and partial)
- [ ] Implement mortgage penalty estimation (variable: 3 months interest; fixed: max of 3 months or IRD)
- [ ] Build API route `GET /api/result/[id]` — fetch assessment result from Supabase
...
- [ ] Add SEO meta tags, Open Graph cards, and social sharing support
- [ ] Implement SearchStrata cross-promotion for condo property types on result page
